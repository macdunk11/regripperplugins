#-----------------------------------------------------------
# typedurlstime.pl
#   Plugin for Registry Ripper, NTUSER.DAT edition - gets the TypedURLs and TypedURLsTime values 
#
# Change history
...
sub pluginmain {
	...
	if (my $key = $root_key->get_subkey($key_path)) 
	{
		...
		if (scalar(@timevals) > 0) 
		{
			...
			::rptMsg("\nTypedURLS/TypedURLsTime Correlated Results");
			::rptMsg("==========================================");
		...
		#Correlate values from TypedURLs and TypedURLsTime
			for(my $i = 0; $i <= 50; $i++)
			{
				...
				{
					::rptMsg("  ".$val1. " -> ".$data1." -> ".gmtime($data2). " UTC");
				}			
			}
		}
		else {
